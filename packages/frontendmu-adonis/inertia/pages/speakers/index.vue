    ...
    <div class="contain relative z-10">
      ...
      <section v-if="featuredSpeakers.length" class="mb-20 space-y-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <div v-for="speaker in featuredSpeakers" :key="speaker.id" class="group relative">
            <SpeakerAvatar
              size="full"
              :name="speaker.name"
      ...
      <section v-if="regularSpeakers.length" class="space-y-8">
        ...
        <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-5 gap-y-8">
          ...
          >
            ...
            <div>
              <h4
                class="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-verse-500 transition-colors truncate"
              >
                {{ speaker.name }}
              </h4>
      ...
      <div v-if="!speakers.length" class="text-center py-32 space-y-6">
        <div
          class="w-20 h-20 bg-verse-50 dark:bg-verse-900/20 rounded-full flex items-center justify-center mx-auto"
        >
          <svg
            class="w-10 h-10 text-verse-500 dark:text-verse-400"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
