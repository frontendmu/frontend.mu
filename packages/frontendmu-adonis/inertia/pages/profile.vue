      ...
      <template v-if="user">
        ...
        <header class="flex flex-col sm:flex-row items-start gap-6 mb-12">
          ...
          <div class="space-y-3 min-w-0">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <template v-if="user.githubUsername || user.linkedinUrl || user.twitterUrl || user.websiteUrl">
                <span class="text-gray-200 dark:text-verse-800">|</span>
                <a v-if="user.githubUsername" :href="`https://github.com/${user.githubUsername}`" target="_blank" class="text-gray-400 hover:text-verse-500 transition-colors" title="GitHub">
                ...
                <a v-if="user.linkedinUrl" :href="user.linkedinUrl" target="_blank" class="text-gray-400 hover:text-verse-500 transition-colors" title="LinkedIn">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
                <a v-if="user.twitterUrl" :href="user.twitterUrl" target="_blank" class="text-gray-400 hover:text-verse-500 transition-colors" title="Twitter">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                <a v-if="user.websiteUrl" :href="user.websiteUrl" target="_blank" class="text-gray-400 hover:text-verse-500 transition-colors" title="Website">
        ...
        <section v-if="rsvpMeetups.length" class="mt-16 space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div class="space-y-2">
                <p class="font-bold text-gray-900 dark:text-gray-100 group-hover:text-verse-500 transition-colors leading-snug">
                  {{ meetup.title }}
                </p>
                <p v-if="meetup.date" class="text-[10px] font-mono text-gray-400 uppercase tracking-widest">
                  {{ meetup.date }}
                </p>
              </div>
            </Link>
