...
const form = useForm({
  ...
  githubUsername: props.speaker.githubUsername || '',
  bio: props.speaker.bio || '',
  linkedinUrl: props.speaker.linkedinUrl || '',
  twitterUrl: props.speaker.twitterUrl || '',
  websiteUrl: props.speaker.websiteUrl || '',
  featured: props.speaker.featured,
})

function handleSubmit() {
  form.put(`/admin/speakers/${props.speaker.id}`)
}
</script>

<template>
  <Head :title="`Edit: ${speaker.name}`" />
  <main class="relative min-h-screen pt-40 pb-20">
      <ContentBlock>
        ...
        <nav class="mb-6 flex items-center gap-2 text-sm">
          ...
          <span class="text-verse-400">/</span>
          <span class="text-verse-900 dark:text-verse-100">{{ speaker.name }}</span>
        </nav>
        ...
        <div class="flex items-center gap-4 mb-8">
          ...
          />
          <div>
            <BaseHeading :level="1">Edit Speaker</BaseHeading>
            <p class="text-verse-600 dark:text-verse-400">{{ speaker.name }}</p>
          </div>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <!-- Sessions -->
          <div v-if="speaker.sessions.length" class="pt-4 border-t border-verse-200 dark:border-verse-700">
            <h3 class="text-sm font-medium text-verse-700 dark:text-verse-300 mb-3">Sessions</h3>
            <div class="space-y-2">
              <div
                v-for="session in speaker.sessions"
                :key="session.id"
                class="flex items-center justify-between p-3 bg-verse-50 dark:bg-verse-800 squircle rounded-lg"
              >
                <div>
