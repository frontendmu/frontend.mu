...
function getMobileFocusableElements() {
  ...
  return Array.from(panel.querySelectorAll<HTMLElement>(MOBILE_FOCUSABLE_SELECTOR))
...
async function focusMobileMenu() {
  ...
  const initialFocus = mobileMenuCloseButton.value
    ?? getMobileFocusableElements()[0]
...
function restoreMobileMenuFocus() {
  if (!shouldRestoreMobileMenuFocus)
    return

  const fallbackFocusTarget = mobileMenuTrigger.value
  const focusTarget = previousFocusedElement && document.contains(previousFocusedElement)
...
<template>
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        <div
          class="absolute inset-0 bg-verse-950/35 backdrop-blur-sm"
          aria-hidden="true"
        ...
        >
          <div
            ...
            ref="mobileMenuPanel"
            class="relative ml-auto flex h-full w-full max-w-sm flex-col overflow-y-auto bg-[linear-gradient(180deg,rgba(255,250,244,0.98),rgba(243,247,255,0.98))] text-gray-900 shadow-2xl backdrop-blur-2xl dark:bg-[linear-gradient(180deg,rgba(2,6,23,0.985),rgba(3,7,18,0.965))] dark:text-gray-100"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div class="pointer-events-none absolute inset-0 overflow-hidden">
              <div class="absolute right-[-20%] top-[-4%] h-56 w-56 rounded-full bg-verse-500/12 blur-3xl dark:bg-verse-400/10" />
              <div class="absolute left-[-18%] top-48 h-40 w-40 rounded-full bg-orange-400/12 blur-3xl dark:bg-orange-300/10" />
            </div>

            <div
              class="relative z-10 px-5 pb-6"
              style="padding-top: calc(env(safe-area-inset-top) + 1.25rem)"
            >
              <div class="flex items-start justify-between gap-4">
                <Link href="/" class="flex min-w-0 items-center gap-3" @click="closeMobileMenu({ restoreFocus: false })">
                  <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-verse-500 text-white shadow-lg shadow-verse-500/20">
                    <Logo class="w-6 h-6" />
                  </div>

                  <div class="min-w-0">
                    <p id="mobile-menu-title" class="truncate text-2xl font-black tracking-tight text-gray-900 dark:text-white">
                      coders<span class="text-verse-500">.mu</span>
                    </p>
                    <p id="mobile-menu-description" class="mt-1 max-w-[15rem] text-sm text-gray-600 dark:text-gray-300">
                ...
                <button
                  ...
                  type="button"
                  class="mobile-focus inline-flex h-11 w-11 items-center justify-center rounded-xl border border-gray-200 bg-white/80 text-gray-500 transition-colors hover:text-verse-500 dark:border-verse-800 dark:bg-verse-900/60 dark:text-gray-300 dark:hover:text-verse-300"
                  aria-label="Close navigation menu"
                  @click="closeMobileMenu"
                >
                  <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.25" d="M6 18 18 6M6 6l12 12" />
                  </svg>
                </button>
              </div>
            </div>
            ...
            <div
              class="relative z-10 px-5 py-4"
              role="group"
              aria-labelledby="mobile-quick-actions-title"
              aria-describedby="mobile-quick-actions-description"
            >
              <div class="sr-only">
                <p id="mobile-quick-actions-title">
                  Quick actions
                </p>
                <p id="mobile-quick-actions-description">
            ...
            <nav aria-label="Mobile navigation" aria-describedby="mobile-navigation-meta" class="relative z-10 flex-1 px-5 pb-6 pt-4">
              ...
              <ul class="border-y border-gray-200/70 dark:border-white/8">
                <template v-for="[key, item] of linkEntries" :key="key">
                  <li class="overflow-visible border-b border-gray-200/65 last:border-b-0 dark:border-white/7">
                    <div class="relative">
                      <span
                      ...
                      <button
                        ...
                        :class="isItemActive(item) ? 'text-gray-950 dark:text-white' : 'text-gray-900 dark:text-gray-100'"
                      ...
                      >
                        ...
                        <div class="flex items-center gap-3">
                          ...
                          <svg
                            ...
                            :class="isSectionExpanded(key) ? 'rotate-180 text-gray-700 dark:text-gray-200' : ''"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.25" d="m19 9-7 7-7-7" />
                          </svg>
                        </div>
                      </button>
                      ...
                      >
                        ...
                        <div class="flex items-center gap-3">
                          ...
                          <svg class="h-4 w-4 shrink-0 text-gray-400 transition-colors duration-200 group-hover:text-gray-600 dark:text-gray-500 dark:group-hover:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      >
                        ...
                        <svg class="h-4 w-4 shrink-0 text-gray-400 transition-colors duration-200 group-hover:text-gray-600 dark:text-gray-500 dark:group-hover:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.25" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                      </a>
                      ...
                      >
                        ...
                        >
                          <ul class="space-y-1">
                            <li v-for="submenu in getMobileSectionLinks(item)" :key="`${key}-${submenu.href}`">
                              <Link
                                ...
                                :class="isRouteActive(submenu.href) ? 'text-verse-700 dark:text-verse-200' : 'text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white'"
...
.mobile-focus:focus-visible {
  box-shadow: 0 0 0 2px var(--color-verse-400), 0 0 0 5px rgba(248, 250, 252, 0.96);
}
...
:where(.dark *, .dark) .mobile-focus:focus-visible {
  box-shadow: 0 0 0 2px var(--color-verse-300), 0 0 0 5px rgba(2, 6, 23, 0.96);
}
</style>
