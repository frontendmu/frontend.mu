...
async function runMigration() {
  ...
  try {
    ...
    const sessionsResult = await client.query(
      'SELECT id, event_id, title FROM sessions ORDER BY event_id, "order"'
    )
    ...
    for (const meetup of meetupsRaw) {
      ...
      for (const [i, rawSession] of rawSessions.entries()) {
        ...
        const speakerCheck = await client.query('SELECT id FROM users WHERE id = $1', [speakerId])
    ...
    // Final summary
    const speakerCount = await client.query(
      'SELECT COUNT(DISTINCT speaker_id) FROM session_speakers'
    )
    const sessionCount = await client.query(
      'SELECT COUNT(DISTINCT session_id) FROM session_speakers'
    )
